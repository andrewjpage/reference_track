...
ok my $repository_to_create  = ReferenceTrack::Repository->new(_dbh => $dbh, name => "test repo", location => 'yet_another_repo.git', short_name => 'ABC2' ), 'initialise a repo to create';
...
ok my $preexisting_repo  = ReferenceTrack::Repository->new(_dbh => $dbh, name => "existing repo", location => 'different_location.git', short_name => 'ABC3' ), 'initialise a repo which has a name that already exists';
